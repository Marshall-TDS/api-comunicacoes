# 🚀 Guia de Deploy - API de Comunicações

Este guia fornece instruções passo a passo para configurar o deploy automático da API de Comunicações no servidor VPS da Hostinger usando Docker e GitHub Actions.

## 🖥️ Informações do Servidor

- **IP do Servidor**: `72.61.223.230`
- **Servidor**: VPS Hostinger
- **Porta Homologação**: `5334`
- **Porta Produção**: `3334`
...
- **Homologação**: Porta `5334` (branch `homolog`)
- **Produção**: Porta `3334` (branch `main`)
...
```bash
sudo mkdir -p /var/www/api-comunicacoes
sudo chown $USER:$USER /var/www/api-comunicacoes
...
- `API_USUARIOS_URL_HOMOLOG` - URL da API de usuários para homologação (ex: `http://localhost:5333/api`)
- `API_USUARIOS_URL_MAIN` - URL da API de usuários para produção (ex: `http://localhost:3333/api`)
...
docker-compose build api-comunicacoes-homolog
docker-compose up -d api-comunicacoes-homolog

# Para produção
git checkout main
git pull origin main
docker-compose build api-comunicacoes-main
...
```bash
docker-compose ps
docker-compose logs api-comunicacoes-homolog
docker-compose logs api-comunicacoes-main
```
...
1. Acesse seu repositório no GitHub
2. Vá em **Actions**
3. Você verá o workflow "Deploy API Comunicações - Homologação" sendo executado
...
```bash
# Logs de homologação
docker-compose logs -f api-comunicacoes-homolog

# Logs de produção
docker-compose logs -f api-comunicacoes-main
```
...
```bash
docker-compose stop api-comunicacoes-homolog
docker-compose stop api-comunicacoes-main
```

### Reiniciar containers

```bash
docker-compose restart api-comunicacoes-homolog
docker-compose restart api-comunicacoes-main
```

### Rebuild completo

```bash
docker-compose build --no-cache api-comunicacoes-homolog
docker-compose up -d api-comunicacoes-homolog
```
...
- Verifique se as variáveis `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASS` estão corretas no GitHub Secrets
...
docker inspect api-comunicacoes-homolog | grep -A 10 Health
```
...
- Push para `homolog` → Deploy automático na porta 5334
- Push para `main` → Deploy automático na porta 3334
